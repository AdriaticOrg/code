report 13062662 "Export KRD"
{
    ...
    begin
        ...
        KRDReportHeader.TestField("Period Start Date");

        GetUser(RespUser,KRDReportHeader."Resp. User ID");
        GetUser(MakerUser,KRDReportHeader."Prep. By User ID");

        SplitAddress(CompanyInfo.Address + CompanyInfo."Address 2",Street,HouseNo);

        StatMonth := DATE2DMY(KRDReportHeader."Period Start Date",2);
        StatYear := DATE2DMY(KRDReportHeader."Period Start Date",3);        
        ...
        KRDReportLine.Reset();
        KRDReportLine.SetRange("Document No.",KRDReportHeader."No.");
    ...
    LOCAL procedure SplitAddress(OrgAddress : Text[200];VAR Street : Text[200];VAR House : Text[100])
    var
    SpaceFound:Boolean;
    Cntr:Integer;
    Len:Integer;
    ...
    local procedure GetUser(var UserSetup:Record "User Setup";UserID:Code[59])
    begin
